...
export default function Page(){
  ...
  const tool=TOOLS.find(x=>x.slug==='svg-viewer')
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs font-semibold text-gray-600">{t('sv_code')}</p>
              <div className="flex gap-2 items-center">
                <label className="flex items-center gap-1 text-xs text-gray-600">{t('sv_bg')}<input type="color" value={bg} onChange={e=>setBg(e.target.value)} className="w-6 h-6 rounded"/></label>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-600 mb-1">{t('htg_preview')}</p>
            <div className="rounded-xl border border-gray-200 flex items-center justify-center overflow-auto min-h-[250px]" style={{backgroundColor:bg}}>
              {valid?<div style={{transform:'scale('+scale+')',transformOrigin:'center center'}} dangerouslySetInnerHTML={{__html:code}}/>:
                <p className="text-red-500 text-sm">{t('stp_invalidsvg')}</p>}
            </div>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={dl} disabled={!valid} className="px-4 py-2 bg-blue-600 text-white rounded text-sm font-medium hover:bg-blue-700 disabled:opacity-50">{t('sv_download')}</button>
          <button onClick={()=>navigator.clipboard?.writeText(code)} className="px-4 py-2 border border-gray-300 rounded text-sm hover:bg-gray-50">{t('sv_copycode')}</button>
          <button onClick={()=>setCode(SAMPLE)} className="px-4 py-2 border border-gray-300 rounded text-sm hover:bg-gray-50">{t('sv_sample')}</button>
        </div>
      </div>
    </ToolLayout>
  )
