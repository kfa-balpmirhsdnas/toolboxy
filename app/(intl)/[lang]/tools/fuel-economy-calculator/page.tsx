  ...
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-lg mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('fe_title')}</h1>
        <p className="text-gray-500 mb-8">{t('fe_subtitle')}</p>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('sp_distance')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('fe_fuel')}</label>
            <div className="flex gap-2">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('fe_price')}</label>
            <div className="flex gap-2">
              <input type="number" value={fuelPrice} onChange={e=>setFuelPrice(e.target.value)} placeholder="e.g. 1.65"
                className="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500" />
              {(['L','gal'] as const).map(u=>(
                <button key={u} onClick={()=>setPriceUnit(u)} className={'px-3 py-2 rounded-lg font-medium transition-colors '+(priceUnit===u?'bg-brand-500 text-white':'bg-gray-100 text-gray-700')}>/{u}</button>
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            ...
            {pricePer100km!==null&&(
              <div className="col-span-3 bg-brand-50 border border-brand-200 rounded-xl p-4 text-center">
                ...
                <div className="text-xs text-gray-500 mt-1">{t('fe_cost100')}</div>
              </div>
            )}
          </div>
        )}
