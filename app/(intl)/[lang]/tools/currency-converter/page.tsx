...
export default function CurrencyConverterPage() {
  ...
  const Sel=({val,set}:{val:string;set:(v:string)=>void})=>(
    <select value={val} onChange={e=>set(e.target.value)} className="w-full rounded border border-gray-300 px-2 py-2 text-sm">
      {Object.keys(RATES).map(c=><option key={c} value={c}>{c} — {t('cur_'+c.toLowerCase())}</option>)}
    </select>
  )
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-md mx-auto px-4 space-y-5">
        ...
        <p className="text-xs text-gray-400 text-center">{t('cv_disclaimer')}</p>
        <div>
          <p className="text-xs font-medium text-gray-600 mb-2">{t('cv_popular')}</p>
          <div className="flex flex-wrap gap-2">
            {POPULAR.map(([f,to2])=>(
              <button key={f+to2} onClick={()=>{setFrom(f);setTo(to2)}}
                className="px-3 py-1.5 rounded-full border border-gray-200 text-xs hover:bg-gray-50">{f}/{to2}</button>
            ))}
          </div>
        </div>
      </div>
