...
const TYPES=[{id:'normal'},{id:'deuteranopia'},{id:'protanopia'},{id:'tritanopia'},{id:'achromatopsia'}]
    ...
    <ToolLayout tool={tool}>
      <div className="max-w-lg mx-auto px-4 space-y-5">
        <div className="space-y-2">
          {TYPES.map(ty=>(
            <button key={ty.id} onClick={()=>setSelected(ty.id)}
              className={'w-full flex items-start gap-3 px-3 py-2.5 rounded-lg border text-left transition '+(selected===ty.id?'bg-blue-50 border-blue-300':'border-gray-200 hover:bg-gray-50')}>
              <div className={'w-4 h-4 rounded-full border-2 flex-shrink-0 mt-0.5 '+(selected===ty.id?'bg-blue-600 border-blue-600':'border-gray-400')}/>
              <div><p className="text-sm font-medium text-gray-800">{t('cb_'+ty.id)}</p><p className="text-xs text-gray-500">{t('cb_'+ty.id+'_d')}</p></div>
            </button>
          ))}
        </div>
        <div>
          <p className="text-sm font-medium text-gray-700 mb-3">{t('cb_test')}</p>
          <div className="flex gap-3 items-center mb-3">
            ...
            <div className="flex-1 flex gap-3">
              <div className="flex-1 text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">{t('cb_original')}</p>
              ...
              <div className="flex-1 text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">{t('cb_'+selected)}</p>
        ...
        <div>
          <p className="text-sm font-medium text-gray-700 mb-3">{t('cb_palette')}</p>
          ...
          <p className="text-xs text-gray-400 mt-2 text-center">{t('cb_legend')}</p>
        </div>
      </div>
    </ToolLayout>
  )
