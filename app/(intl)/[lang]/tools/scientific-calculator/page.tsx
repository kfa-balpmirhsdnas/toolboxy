...
export default function ScientificCalculatorPage() {
  ...
  const calc=()=>{
    try{
      ...
      const r=fn((v: number)=>Math.sin(toRad(v)),(v: number)=>Math.cos(toRad(v)),(v: number)=>Math.tan(toRad(v)),Math)
      const rs=typeof r==='number'?parseFloat(r.toFixed(10)).toString():t('sci_error')
      setResult(rs);setHistory(h=>[expr+' = '+rs,...h.slice(0,9)])
    }catch{setResult(t('sci_error'))}
  }
  const ap=(v: string)=>setExpr(e=>e+v)
  const rows=[['sin(','cos(','tan(','π'],['7','8','9','/'],[' 4','5','6','*'],['1','2','3','-'],['0','.','**','+'],[' sqrt(','log(','ln(','('],[')','^','C','⌫']]
  return (<ToolLayout tool={tool}><div className="max-w-sm mx-auto px-4"><div className="bg-gray-900 rounded-xl p-4 space-y-3"><div className="flex justify-between items-center"><span className="text-gray-400 text-sm">{t('sci_title')}</span><button onClick={()=>setDeg(d=>!d)} className="text-sm bg-gray-700 text-white px-3 py-1 rounded">{deg?'DEG':'RAD'}</button></div><input value={expr} onChange={e=>setExpr(e.target.value)} onKeyDown={e=>e.key==='Enter'&&calc()} className="w-full bg-gray-800 text-white text-right text-lg p-3 rounded-lg font-mono border border-gray-700 focus:outline-none" placeholder={t('sci_ph')}/><div className="bg-gray-800 text-green-400 text-right text-2xl p-3 rounded-lg font-mono min-h-14 flex items-center justify-end">{result||'0'}</div><div className="grid grid-cols-4 gap-2">{rows.flat().map((b,i)=>{const bt=b.trim();return(<button key={i} onClick={bt==='C'?()=>{setExpr('');setResult('')}:bt==='⌫'?()=>setExpr(e=>e.slice(0,-1)):()=>ap(bt)} className={'p-3 rounded-lg text-sm font-semibold '+(bt==='C'?'bg-red-600 text-white hover:bg-red-700':'bg-gray-700 text-white hover:bg-gray-600')}>{bt}</button>)})}<button onClick={calc} className="col-span-4 p-3 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700">=</button></div></div>{history.length>0&&(<div className="mt-4 space-y-1"><p className="text-xs font-semibold text-gray-500">{t('sci_history')}</p>{history.map((h,i)=><p key={i} className="text-sm font-mono text-gray-600">{h}</p>)}</div>)}</div></ToolLayout>)
}