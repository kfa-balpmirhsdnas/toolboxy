...
export default function PdfToPngPage({ params }: { params: { lang: string } }) {
  ...
  function download(dataUrl: string, i: number) {
    ...
    a.href = dataUrl
    a.download = (file?.name.replace(/\.pdf$/i, '') || 'page') + `-${i + 1}.png`
    a.click()
    trackToolDownload('pdf-to-png', 'png')
  }
    ...
    <ToolLayout tool={tool} lang={params.lang}>
      <div className="space-y-4">
        <div onClick={() => inputRef.current?.click()}
          ...
          <p className="text-xs text-gray-400 mt-1">{t('pn_sub')}</p>
