...
export default function Page(){
  ...
  const tool=TOOLS.find(x=>x.slug==='gradient-text-generator')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-lg mx-auto px-4 space-y-4">
        <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('btc_text')}</label>
          <input value={text} onChange={e=>setText(e.target.value)} className="w-full rounded border border-gray-300 px-3 py-2"/></div>
        <div className="grid grid-cols-3 gap-3">
          <label className="flex flex-col gap-1 text-xs text-gray-600">{t('cmx_c1')}<input type="color" value={c1} onChange={e=>setC1(e.target.value)} className="w-full h-9 rounded cursor-pointer"/></label>
          <label className="flex flex-col gap-1 text-xs text-gray-600">{t('cmx_c2')}<input type="color" value={c2} onChange={e=>setC2(e.target.value)} className="w-full h-9 rounded cursor-pointer"/></label>
          <label className="flex flex-col gap-1 text-xs text-gray-600">{t('gtg_c3')}<input type="color" value={c3||'#ffffff'} onChange={e=>setC3(e.target.value)} className="w-full h-9 rounded cursor-pointer"/></label>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <label className="flex flex-col gap-1 text-xs text-gray-600">{t('cgg_angle')}
          ...
          <label className="flex flex-col gap-1 text-xs text-gray-600">{t('gtg_fontsize')}
          ...
          <label className="flex flex-col gap-1 text-xs text-gray-600">{t('gtg_weight')}
