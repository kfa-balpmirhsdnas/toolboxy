...
export default function UuidBulkGeneratorPage() {
  ...
  const FORMATS=[{id:'default',label:'ubg_default'},{id:'upper',label:'ubg_upper'},{id:'no-dash',label:'ubg_nodash'},{id:'braces',label:'ubg_braces'}]
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-lg mx-auto px-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('ubg_count')}: {count}</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('ubg_format')}</label>
            <div className="grid grid-cols-2 gap-1">
              {FORMATS.map(f=>(
                <button key={f.id} onClick={()=>setFormat(f.id as typeof format)}
                  className={`py-1.5 rounded border text-xs font-medium transition ${format===f.id?'bg-blue-600 text-white border-blue-600':'border-gray-300 hover:bg-gray-50'}`}>{t(f.label)}</button>
              ))}
            </div>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={generate} className="flex-1 bg-blue-600 text-white rounded-lg py-2.5 font-semibold hover:bg-blue-700">{t('ubg_generate',{n:count})}</button>
          <button onClick={copyAll} className="px-4 py-2.5 rounded-lg border border-gray-300 text-sm hover:bg-gray-50">{copied?t('ui_copied'):t('ui_copy_all')}</button>
        </div>
        <div className="max-h-80 overflow-y-auto space-y-1">
          {uuids.map((u,i)=>(
            <div key={i} className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded hover:bg-gray-100 group">
              ...
              <button onClick={()=>navigator.clipboard.writeText(fmt(u))} className="text-xs text-blue-500 opacity-0 group-hover:opacity-100">{t('ui_copy')}</button>
            </div>
          ))}
        </div>
      </div>
