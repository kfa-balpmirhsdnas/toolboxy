...
export default function Page(){
  ...
  const tool=TOOLS.find(x=>x.slug==='dice-roller')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-sm mx-auto px-4 space-y-5 text-center">
        ...
        <label className="flex items-center gap-2 justify-center text-sm text-gray-700">{t('dr_count')}
        ...
        <button onClick={roll} disabled={rolling} className="px-8 py-4 bg-blue-600 text-white rounded-2xl font-bold text-lg hover:bg-blue-700 disabled:opacity-60 active:scale-95">
          {rolling?t('dr_rolling'):t('dr_roll')+' d'+sides}</button>
        {rolls.length>0&&<>
          ...
          {count>1&&<p className="text-lg font-bold">{t('tc_total')}: <span className="text-blue-600">{total}</span></p>}
        </>}
      </div>
    </ToolLayout>
  )
