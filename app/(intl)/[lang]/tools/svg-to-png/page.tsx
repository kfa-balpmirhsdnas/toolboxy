...
export default function SvgToPngPage({ params }: { params: { lang: string } }) {
  ...
  function convert() {
    ...
    try {
      ...
      if (!svgEl) { setError(t('stp_invalidsvg')); setLoading(false); return }
      ...
      img.onerror = () => { setError(t('stp_renderfail')); setLoading(false) }
      img.src = url
    } catch (e: unknown) {
  ...
  return (
    <ToolLayout tool={tool} lang={params.lang}>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">{t('stp_upload')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">{t('stp_paste')}</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">{t('stp_scale',{s:scale,d:dims.w > 0 ? dims.w + 'x' + dims.h + 'px' : '?'})}</label>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-center">
              <button onClick={download} className="px-5 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700">
                &#x2B07; {t('stp_download',{w:dims.w,h:dims.h})}
              </button>
            </div>
          </div>
        )}
