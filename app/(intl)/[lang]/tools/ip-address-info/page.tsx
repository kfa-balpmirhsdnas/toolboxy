...
export default function IpAddressInfoPage() {
  ...
  const [myIp,setMyIp]=useState(t('ipi_loading'))
  useEffect(()=>{
    fetch('https://api.ipify.org?format=json').then(r=>r.json()).then(d=>setMyIp(d.ip)).catch(()=>setMyIp(t('ipi_unavailable')))
  },[])
  const ip=input||myIp
  ...
  const infos=isV4?[
    ['ipi_version','IPv4'],['ipi_class',ipClass(ip)],['ipi_type',isPrivate(ip)?t('ipi_private'):t('ipi_public')],['ipi_loopback',ip.startsWith('127.')?t('ipi_yes'):t('ipi_no')],['ipi_broadcast',ip.endsWith('.255')?t('ipi_yes'):t('ipi_no')],['ipi_binary',toBinary(ip)],
  ]:isV6?[['ipi_version','IPv6'],['ipi_type','IPv6 address']]:[]
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-md mx-auto px-4 space-y-4">
        <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('ipi_label')}</label>
          <input value={input} onChange={e=>setInput(e.target.value)}
            ...
            placeholder={t('ipi_ph')}/>
          {input&&!valid&&<p className="text-red-500 text-xs mt-1">{t('ipi_invalid')}</p>}
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center justify-between">
          <div><p className="text-xs text-blue-600 font-medium">{t('ipi_current')}</p>
            <p className="font-mono font-bold text-blue-800 text-lg">{myIp}</p></div>
          <button onClick={()=>setInput(myIp)} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700">{t('ipi_usethis')}</button>
        </div>
          ...
          <div className="bg-gray-50 rounded-xl overflow-hidden border border-gray-200">
            {infos.map(([k,v],i)=>(
              <div key={i} className={'flex items-start justify-between px-4 py-2.5 '+(i%2===0?'bg-white':'bg-gray-50')}>
                <span className="text-xs font-medium text-gray-500 w-24">{t(k)}</span>
                <span className="font-mono text-sm text-gray-800 text-right break-all">{v}</span>
              </div>
            ))}
          </div>
        )}
        <div><p className="text-xs font-semibold text-gray-600 mb-2">{t('ipi_ranges')}</p>
