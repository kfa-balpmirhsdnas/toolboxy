...
export default function BlockPuzzlePage({ params }: { params: { lang: string } }) {
  ...
  const move = (dx: number) => { const p = pieceRef.current; if (!p || !playing) return; const np = { ...p, x: p.x + dx }; if (!collides(board, np)) { pieceRef.current = np; force((n) => n + 1) } }
  const rot = () => { const p = pieceRef.current; if (!p || !playing) return; const m = rotate(p.m); for (const k of [0, -1, 1, -2, 2]) { const c = { ...p, m, x: p.x + k }; if (!collides(board, c)) { pieceRef.current = c; force((n) => n + 1); return } } }
  const hard = () => { const p = pieceRef.current; if (!p || !playing) return; let np = { ...p }; while (!collides(board, { ...np, y: np.y + 1 })) np = { ...np, y: np.y + 1 }; lockAndNext(board, np) }
