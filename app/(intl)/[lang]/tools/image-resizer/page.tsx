        ...
        {src&&(
          <>
            <div className="flex gap-3 items-end">
              <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">{t('ir_width')}</label>
              ...
              <button onClick={()=>setLock(l=>!l)} className={'px-3 py-2.5 rounded border text-sm transition '+(lock?'bg-blue-600 text-white border-blue-600':'border-gray-300 text-gray-600 hover:bg-gray-50')}>{lock?t('ir_lock'):t('ir_free')}</button>
              <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">{t('ir_height')}</label>
            ...
            <div className="flex gap-3 flex-wrap">
              <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">{t('ubg_format')}</label>
              ...
              {format!=='image/png'&&<div className="flex-1"><label className="block text-xs text-gray-500 mb-1">{t('ir_quality')}: {Math.round(quality*100)}%</label>
            ...
            <button onClick={resize} className="w-full py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition">{t('ir_resize')}</button>
            {resized&&(
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-sm text-gray-600">{t('ir_result')}: <span className="font-semibold">{w}x{h}px</span> — ~{resizedSize}KB</p>
                  <a href={resized} download={'resized.'+format.split('/')[1]} className="text-sm text-blue-600 hover:underline">{t('ui_download')}</a>
                </div>
