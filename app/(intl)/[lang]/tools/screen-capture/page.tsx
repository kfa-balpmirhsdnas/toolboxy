...
// access as screen-recorder). Capture (now or after a 5s countdown) → edit (crop,
// arrow/rect/text annotations, mosaic) → PNG/JPG download or clipboard copy.
// The 5s mode lets the user hide the browser's screen-share bar before the grab.

import { useState, useRef, useEffect } from 'react'
import { useTranslations } from 'next-intl'
import ToolLayout from '@/components/tools/ToolLayout'
import ScreenCaptureEditor from '@/components/tools/ScreenCaptureEditor'
...
export default function ScreenCapturePage({ params }: { params: { lang: string } }) {
  ...
  const canvasRef = useRef<HTMLCanvasElement | null>(null) // the original (un-edited) capture, for auto-save
  ...
  async function autoSavePng() {
    ...
    trackToolDownload('screen-capture', 'png')
  }

  async function capture() {
    setError(''); setAutoSaved(false); setBusy(true)
    ...
    try {
      ...
      // frame arrive. setTimeout fires even in a background tab (unlike rAF).
      ...
      // can't reach the editor — auto-save the original PNG as a safety net. Only
      // 'window' keeps the page visible; an unknown surface falls back to saving.
      if (surface !== 'window') {
        setAutoSaved(true)
        autoSavePng()
      }
        ...
        <div className="space-y-4">
          ...
          {!captured ? (
            ...
            <div className="space-y-3">
              ...
              {delayMode && <p className="text-xs text-gray-500">⏱ {t('sc_delay_hint')}</p>}
              <ScreenCaptureEditor source={captured} onRecapture={capture} timingToggle={timingToggle} />
            </div>
          )}
