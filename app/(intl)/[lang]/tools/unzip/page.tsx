...
export default function UnzipPage({ params }: { params: { lang: string } }) {
  ...
  // Android "Share → ToolBoxy Unzip" (installed PWA): open the .zip shared to us.
  useSharedFile(load)
      ...
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className={'space-y-4 transition-opacity ' + (entries.length ? '' : 'opacity-50 pointer-events-none select-none')} aria-disabled={!entries.length}>
          ...
          <div className="rounded-xl border border-gray-100 overflow-hidden">
            {/* Column titles */}
            <div className={'flex items-center gap-2 px-4 py-2 bg-gray-50 text-xs font-medium text-gray-500' + (entries.length > 0 ? ' border-b border-gray-100' : '')}>
