...
export default function BudgetCalculatorPage() {
  ...
  const cat=(c:string)=>t('bg_c_'+c.toLowerCase())
  const [items,setItems]=useState<Item[]>(()=>[
    {id:_id++,name:t('bg_d_salary'),amount:'5000',category:'Salary',type:'income'},
    {id:_id++,name:t('bg_d_rent'),amount:'1500',category:'Housing',type:'expense'},
    {id:_id++,name:t('bg_d_groceries'),amount:'400',category:'Food',type:'expense'},
    {id:_id++,name:t('bg_d_transport'),amount:'150',category:'Transport',type:'expense'},
  ])
  ...
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('bg_title')}</h1>
        <p className="text-gray-500 mb-6">{t('bg_subtitle')}</p>
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[['bg_income_total',income,'text-green-600','bg-green-50'],['bg_expense_total',expense,'text-red-500','bg-red-50'],['bg_balance',balance,balance>=0?'text-blue-600':'text-red-600',balance>=0?'bg-blue-50':'bg-red-50']].map(([l,v,tc,bg])=>(
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-800 mb-3">{t('bg_income')}</h2>
          {items.filter(i=>i.type==='income').map(i=>(
            <div key={i.id} className="flex gap-2 items-center mb-2">
              ...
              <select value={i.category} onChange={e=>update(i.id,'category',e.target.value)} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm">
                {CATS_INCOME.map(c=><option key={c} value={c}>{cat(c)}</option>)}
              </select>
          ...
          <h2 className="font-semibold text-gray-800 mt-4 mb-3">{t('bg_expenses')}</h2>
          {items.filter(i=>i.type==='expense').map(i=>(
            <div key={i.id} className="flex gap-2 items-center mb-2">
              <input value={i.name} onChange={e=>update(i.id,'name',e.target.value)} className="flex-1 border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none" />
              <select value={i.category} onChange={e=>update(i.id,'category',e.target.value)} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm">
                {CATS_EXPENSE.map(c=><option key={c} value={c}>{cat(c)}</option>)}
              </select>
              <input type="number" value={i.amount} onChange={e=>update(i.id,'amount',e.target.value)} className="w-24 border border-gray-200 rounded-lg px-2 py-1.5 text-sm font-mono text-right focus:outline-none" />
              <button onClick={()=>remove(i.id)} className="text-gray-300 hover:text-red-400">\u00D7</button>
            </div>
          ...
          <div className="border-t border-gray-100 mt-4 pt-4">
            <p className="text-xs font-medium text-gray-500 mb-2">{t('bg_add')}</p>
            <div className="flex gap-2">
              <select value={newType} onChange={e=>setNewType(e.target.value as 'income'|'expense')} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm">
                <option value="income">{t('bg_income')}</option><option value="expense">{t('bg_expense_opt')}</option>
              </select>
              <input value={newName} onChange={e=>setNewName(e.target.value)} placeholder={t('bg_name_ph')} className="flex-1 border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none" />
              <select value={newCat} onChange={e=>setNewCat(e.target.value)} className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm">
                {(newType==='income'?CATS_INCOME:CATS_EXPENSE).map(c=><option key={c} value={c}>{cat(c)}</option>)}
              </select>
        ...
        {Object.keys(expBycat).length>0&&(
          <div className="mt-4 bg-white rounded-2xl border border-gray-200 p-5">
            <h2 className="font-semibold text-gray-800 mb-3">{t('bg_breakdown')}</h2>
            {Object.entries(expBycat).sort((a,b)=>b[1]-a[1]).map(([c,amt])=>(
              <div key={c} className="mb-2">
                <div className="flex justify-between text-sm mb-0.5">
                  <span className="text-gray-700">{cat(c)}</span>
