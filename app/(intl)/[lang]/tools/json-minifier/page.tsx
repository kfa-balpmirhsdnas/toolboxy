...
export default function Page(){
  ...
  const tool=TOOLS.find(x=>x.slug==='json-minifier')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        <div className="flex gap-2">
          {[['minify','jf_minify'],['format','jm_format']].map(([m,lbl])=>(
            <button key={m} onClick={()=>setMode(m)} className={'px-4 py-2 rounded text-sm font-medium border '+(mode===m?'bg-blue-600 text-white border-blue-600':'border-gray-300 hover:bg-gray-50')}>{t(lbl)}</button>
          ))}
          {mode==='format'&&<label className="flex items-center gap-2 text-sm text-gray-700 ml-2">{t('jm_indent')}
            <select value={indent} onChange={e=>setIndent(+e.target.value)} className="rounded border border-gray-300 px-2 py-1.5 text-sm">
              {[2,4,8].map(n=><option key={n} value={n}>{n} {t('jm_spaces')}</option>)}
            </select></label>}
        </div>
        ...
        {!error&&output&&<>
          <div className="flex justify-between text-xs text-gray-500 bg-gray-50 rounded px-3 py-2">
            <span>{input.length} → {output.length} {t('ui_chars')}</span>
            {mode==='minify'&&<span className="text-green-600 font-semibold">{pct}% {t('hm_saved')}</span>}
          </div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('ui_output')}</label>
          ...
          <button onClick={()=>navigator.clipboard?.writeText(output)} className="px-4 py-2 bg-blue-600 text-white rounded text-sm font-medium hover:bg-blue-700">{t('ui_copy')}</button>
        </>}
      </div>
    </ToolLayout>
  )
