...
export default function MusicPlayerPage({ params: { lang } }: { params: { lang: string } }) {
  ...
  return (
    <ToolLayout tool={tool} lang={lang}>
      <div className="space-y-4 max-w-lg min-[700px]:max-w-none mx-auto">
            ...
            <div ref={cardRef} className={'min-[700px]:order-2 rounded-2xl text-white shadow-sm overflow-hidden scroll-mt-16 bg-gradient-to-b ' + (darkMode ? 'from-gray-800 to-black' : 'from-brand-500 to-brand-700')}>
              ...
              <div className="flex items-center justify-center gap-3 mt-3 text-white">
                ...
                <button onClick={() => {
                  ...
                  if (!r) return
                  // Near-full-height panel: from just under the fixed header down to a small bottom
                  // margin (the centered player-sized version left too much empty space above).
                  const headerSafe = 64 // fixed header (h-14) + breathing room
                  setListOv({ top: headerSafe, left: r.left, w: r.width, h: window.innerHeight - headerSafe - 16 })
              ...
              {histTab === 'all' ? (
                ...
                <div className={(listOv ? 'flex-1 min-h-0 flex flex-col ' : '') + 'min-[700px]:flex-1 min-[700px]:min-h-0 min-[700px]:flex min-[700px]:flex-col'}>
                  <div className={'flex items-center gap-1 px-2 py-2 border-b ' + (darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-100')}>
                    ...
                    {/* 곡 추가 first, then the ≡ reorder toggle (positions swapped) */}
                    ...
                    {/* Delete this playlist (only for custom ones; 즐겨찾기 can't be deleted) — swapped before ≡ */}
