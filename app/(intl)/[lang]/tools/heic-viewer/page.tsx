  ...
  return (
    <ToolLayout tool={tool} lang={lang}>
      <div ref={viewerRef} className={'flex flex-col gap-2 ' + (fs ? 'fixed inset-0 z-50 bg-gray-900 p-3' : '')}>
        {/* Photo list — moved to the TOP; dimmed + non-interactive until photos load. */}
        <div className={'space-y-2 transition-opacity ' + (items.length ? '' : 'opacity-50 pointer-events-none select-none')} aria-disabled={!items.length}>
