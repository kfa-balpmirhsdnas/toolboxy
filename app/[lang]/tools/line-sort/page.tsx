...
export default function LineSortPage({ params }: { params: { lang: string } }) {
  ...
  const MODES: [SortMode,string][] = [['az','ls_az'],['za','ls_za'],['length-asc','ls_shortfirst'],['length-desc','ls_longfirst'],['reverse','ls_reverse'],['shuffle','ls_shuffle'],['dedup','ls_dedup']]
    ...
    <ToolLayout tool={tool} lang={params.lang}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">{t('ls_inlines',{n:input.split('\n').filter(l=>l.trim()).length})}</label>
            <textarea value={input} onChange={e=>{setInput(e.target.value);track()}} placeholder={t('ls_ph')} rows={10}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-medium text-gray-600">{t('ls_outlines',{n:output.split('\n').filter(l=>l).length})}</label>
            ...
            <div className="w-full px-3 py-3 border border-gray-200 rounded-xl text-sm min-h-[16rem] bg-gray-50 whitespace-pre-wrap break-words max-h-96 overflow-y-auto">
              {output || <span className="text-gray-400">{t('ls_outph')}</span>}
            </div>
          </div>
        </div>
      </div>
