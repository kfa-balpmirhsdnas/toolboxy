  ...
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('cc_title')}</h1>
        <p className="text-gray-500 mb-8">{t('cc_subtitle')}</p>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <label className="block text-sm font-medium text-gray-700 mb-1">{t('cc_input')}</label>
          ...
          {CASES.map(({id,label,fn})=>{
            ...
            return (
              <div key={id} className="bg-white rounded-xl border border-gray-200 px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 font-mono text-sm text-gray-800 truncate">{val||<span className="text-gray-300">{t('cc_result_ph')}</span>}</div>
