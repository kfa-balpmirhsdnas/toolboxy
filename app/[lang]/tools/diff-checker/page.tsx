        ...
        {!showDiff ? (
          <>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <textarea value={textA} onChange={e=>setTextA(e.target.value)} placeholder="Paste original text..." rows={10}
              ...
              <div>
                ...
                <textarea value={textB} onChange={e=>setTextB(e.target.value)} placeholder="Paste modified text..." rows={10}
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-brand-400 resize-none" />
              </div>
            </div>
          ...
          <>
            <div className="flex items-center gap-4">
              <span className="px-3 py-1 bg-green-100 text-green-700 text-xs rounded-full font-medium">+{added} added</span>
              <span className="px-3 py-1 bg-red-100 text-red-700 text-xs rounded-full font-medium">-{removed} removed</span>
              <button onClick={()=>setShowDiff(false)} className="ml-auto text-xs text-brand-600 hover:underline">Edit</button>
            </div>
