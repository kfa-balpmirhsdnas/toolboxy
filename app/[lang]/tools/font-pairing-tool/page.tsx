...
const PAIRS=[
  {h:'Playfair Display',b:'Source Sans Pro',t:'Elegant'},
  {h:'Montserrat',b:'Merriweather',t:'Modern'},
  {h:'Oswald',b:'Lato',t:'Strong'},
  {h:'Raleway',b:'Roboto',t:'Clean'},
  {h:'Lora',b:'Open Sans',t:'Classic'},
  {h:'Abril Fatface',b:'Poppins',t:'Bold'},
]

export default function Page(){
  const tool=TOOLS.find(t=>t.slug==='font-pairing-tool')
    ...
    <ToolLayout tool={tool}>
      ...
      <div className='space-y-4'>
        <div className='grid grid-cols-2 md:grid-cols-3 gap-2'>
          {PAIRS.map((pair,i)=>(
            <button key={i} onClick={()=>setSel(i)}
              ...
              <span className='text-xs bg-gray-100 px-1 rounded'>{pair.t}</span>
            </button>
          ))}
        </div>
        <div className='flex gap-4'>
          <input value={htxt} onChange={e=>setHtxt(e.target.value)} placeholder='Heading text'
            className='border rounded px-3 py-2 flex-1 text-sm'/>
          <input value={btxt} onChange={e=>setBtxt(e.target.value)} placeholder='Body text'
            className='border rounded px-3 py-2 flex-1 text-sm'/>
        </div>
        <div className='border rounded p-6 bg-white space-y-4'>
          ...
          <p className='text-xs text-gray-400'>Heading: {p.h} | Body: {p.b}</p>
        </div>
      </div>
    </ToolLayout>
  )
