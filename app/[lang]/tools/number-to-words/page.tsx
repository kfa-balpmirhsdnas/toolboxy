    ...
    <ToolLayout tool={tool}>
      <div className="max-w-md mx-auto px-4 space-y-4">
        <div><label className="block text-sm font-medium text-gray-700 mb-1">Number</label>
          ...
          {!valid&&input&&<p className="text-red-500 text-xs mt-1">Must be an integer up to 999 trillion</p>}
        </div>
        {valid&&(
          <>
            <div className="space-y-3">
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="flex items-start justify-between gap-2">
                  <div><p className="text-xs font-medium text-gray-500 mb-1">Cardinal</p>
                    <p className="text-sm font-semibold text-gray-800 capitalize">{words}</p></div>
                  <button onClick={()=>copy(words)} className="flex-shrink-0 text-xs text-blue-500 hover:text-blue-700">{copied===words?'✓ Copied':'Copy'}</button>
                </div>
              </div>
              {ordinal&&<div className="bg-gray-50 rounded-xl p-4">
                <div className="flex items-start justify-between gap-2">
                  <div><p className="text-xs font-medium text-gray-500 mb-1">Ordinal</p>
                    <p className="text-sm font-semibold text-gray-800 capitalize">{ordinal}</p></div>
                  <button onClick={()=>copy(ordinal)} className="flex-shrink-0 text-xs text-blue-500 hover:text-blue-700">{copied===ordinal?'✓ Copied':'Copy'}</button>
                </div>
              </div>}
            </div>
