...
const ZONES = [
  { name: 'thr_z_warmup', lo: 0.5, hi: 0.6 },
  { name: 'thr_z_fatburn', lo: 0.6, hi: 0.7 },
  { name: 'thr_z_aerobic', lo: 0.7, hi: 0.8 },
  { name: 'thr_z_anaerobic', lo: 0.8, hi: 0.9 },
  { name: 'thr_z_max', lo: 0.9, hi: 1.0 },
]
    ...
    <ToolLayout tool={tool} lang={params.lang}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('thr_age')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('thr_rest')}</label>
        ...
        {max && (
          <div className="space-y-2">
            <div className="bg-brand-50 border border-brand-100 rounded-xl p-4 text-center">
              <span className="text-sm text-brand-600">{t('thr_maxhr')} </span>
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              {ZONES.map((z) => (
                <div key={z.name} className="flex items-center justify-between px-4 py-2.5 text-sm border-b border-gray-100 last:border-0">
                  <span className="text-gray-700">{t(z.name)} <span className="text-gray-400">({z.lo * 100}–{z.hi * 100}%)</span></span>
                  <span className="font-semibold text-gray-900">{hr(z.lo)}–{hr(z.hi)} bpm</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-gray-400">{r > 0 ? t('thr_karvonen') : t('thr_pctmax')} · {t('thr_estimate')}</p>
          </div>
        )}
      </div>
