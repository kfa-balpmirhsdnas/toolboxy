...
const EXAMPLES=[
  ...
  {name:'Heart',d:'M 50 30 C 50 28, 48 20, 35 20 C 20 20, 20 38, 20 38 C 20 55, 35 65, 50 80 C 65 65, 80 55, 80 38 C 80 38, 80 20, 65 20 C 52 20, 50 28, 50 30 Z'},
  {name:'Star',d:'M 50 5 L 61 35 L 95 35 L 68 57 L 79 91 L 50 70 L 21 91 L 32 57 L 5 35 L 39 35 Z'},
  {name:'Arrow',d:'M 10 50 L 60 50 L 60 30 L 90 50 L 60 70 L 60 50'},
  {name:'Spiral (arc)',d:'M 50 50 m -30 0 a 30 30 0 1 1 60 0 a 20 20 0 1 0 -40 0 a 10 10 0 1 1 20 0'},
]
      ...
      <div className="max-w-xl mx-auto px-4 space-y-4">
        <div className="flex flex-wrap gap-2">
          {EXAMPLES.map(e=>(
            <button key={e.name} onClick={()=>setD(e.d)}
              className="px-3 py-1.5 rounded-full border border-gray-200 text-xs hover:bg-gray-50">{e.name}</button>
          ))}
        </div>
        <div><label className="block text-sm font-medium text-gray-700 mb-1">SVG path d attribute</label>
        ...
        <div className="flex gap-4 flex-wrap items-center">
          ...
          <label className="flex items-center gap-1.5 text-xs cursor-pointer">
            <input type="checkbox" checked={showFill} onChange={e=>setShowFill(e.target.checked)} className="rounded"/>Fill
          </label>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl p-4 flex items-start justify-between gap-3">
          ...
          <button onClick={copy} className="flex-shrink-0 text-blue-400 hover:text-blue-300 text-xs">{copied?'Copied!':'Copy'}</button>
        </div>
      </div>
    </ToolLayout>
  )
