  ...
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-md mx-auto px-4 space-y-5">
        <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('pal_label')}</label>
        ...
        {text&&(
          <div className={`rounded-2xl p-6 text-center ${isPalin?'bg-green-50 border-2 border-green-400':'bg-red-50 border-2 border-red-400'}`}>
            ...
            <p className={`text-xl font-bold ${isPalin?'text-green-700':'text-red-700'}`}>
              {isPalin?t('pal_yes'):t('pal_no')}
            </p>
            {norm.length>0&&<div className="mt-3 text-xs font-mono space-y-1">
              <p className="text-gray-500">{t('pal_normalized')} <span className="text-gray-700">{norm}</span></p>
              <p className="text-gray-500">{t('pal_reversed')} <span className="text-gray-700">{reversed}</span></p>
            </div>}
          </div>
        )}
        <div>
          <p className="text-xs text-gray-500 mb-2">{t('pal_examples')}</p>
