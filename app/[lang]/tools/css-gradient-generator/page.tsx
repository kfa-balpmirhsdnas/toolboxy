...
const PRESETS=[
  {name:'Sunset',type:'linear' as GradType,angle:135,stops:[{color:'#f97316',position:0},{color:'#ec4899',position:50},{color:'#8b5cf6',position:100}]},
  {name:'Ocean',type:'linear' as GradType,angle:180,stops:[{color:'#06b6d4',position:0},{color:'#3b82f6',position:50},{color:'#6366f1',position:100}]},
  {name:'Forest',type:'linear' as GradType,angle:90,stops:[{color:'#84cc16',position:0},{color:'#22c55e',position:100}]},
  {name:'Fire',type:'radial' as GradType,angle:0,stops:[{color:'#fef08a',position:0},{color:'#f97316',position:50},{color:'#dc2626',position:100}]},
  {name:'Monochrome',type:'linear' as GradType,angle:0,stops:[{color:'#f8fafc',position:0},{color:'#1e293b',position:100}]},
]
      ...
      <div className="max-w-xl mx-auto px-4 space-y-4">
          ...
          <div className="flex rounded overflow-hidden border border-gray-300">
            {(['linear','radial','conic'] as const).map(t=>(
              <button key={t} onClick={()=>setType(t)}
                className={'px-3 py-1.5 text-xs font-medium capitalize transition '+(type===t?'bg-blue-600 text-white':'bg-white text-gray-700 hover:bg-gray-50')}>{t}</button>
            ))}
          </div>
          {type!=='radial'&&(
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <button onClick={addStop} className="text-xs text-blue-600 border border-blue-200 px-2 py-0.5 rounded hover:bg-blue-50">+ Add</button>
          </div>
          {stops.map((s,i)=>(
