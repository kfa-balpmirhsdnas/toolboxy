    ...
    <main className="min-h-screen bg-gray-50 py-10">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Paste Text</label>
            <textarea value={input} onChange={e=>setInput(e.target.value)} rows={8}
              placeholder="Paste any text containing email addresses here..."
        ...
        {emails.length>0&&(
          <div className="mt-6 bg-white rounded-2xl border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-3">
              <span className="font-semibold text-gray-700">{emails.length} Email{emails.length!==1?'s':''} Found</span>
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {emails.map((e,i)=>(
                <div key={i} className="flex items-center justify-between px-3 py-2 bg-gray-50 hover:bg-brand-50 rounded-lg group">
                  ...
                  <button onClick={()=>navigator.clipboard.writeText(e)} className="text-xs text-gray-400 hover:text-brand-600 opacity-0 group-hover:opacity-100 transition-opacity">Copy</button>
                </div>
              ))}
            </div>
          </div>
