...
function strengthLabel(pw: string) {
  ...
  if (score <= 2) return { label: 'Weak', color: 'text-red-500', width: 'w-1/4', bg: 'bg-red-400' }
  if (score <= 4) return { label: 'Fair', color: 'text-yellow-600', width: 'w-2/4', bg: 'bg-yellow-400' }
  if (score <= 5) return { label: 'Strong', color: 'text-green-600', width: 'w-3/4', bg: 'bg-green-400' }
  return { label: 'Very Strong', color: 'text-green-700', width: 'w-full', bg: 'bg-green-500' }
}
  ...
  return (
    <ToolLayout tool={tool} lang={params.lang}>
      <div className="space-y-5">
        ...
        <div>
          <div className="flex justify-between text-sm text-gray-700 mb-2"><span>Length</span><span className="font-semibold">{length}</span></div>
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm text-gray-600 shrink-0">Generate</label>
        ...
        {passwords.length > 1 && (
          <div className="space-y-2">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">All Generated</p>
            {passwords.map((pw, i) => (
              <div key={i} className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                ...
                <button onClick={() => copy(pw)} className="text-xs text-gray-400 hover:text-brand-600 shrink-0">Copy</button>
              </div>
            ))}
          </div>
        )}
