      ...
      <div className="max-w-md mx-auto px-4 space-y-5">
        ...
        <div>
          <div className="flex justify-between text-sm mb-2">
            <label className="font-medium text-gray-700">Tip Percentage</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-blue-50 rounded-xl p-4 text-center">
            <p className="text-xs text-blue-500 font-medium">Tip Amount</p>
            <p className="text-2xl font-bold text-blue-700 mt-1">{fmt(tipAmt)}</p>
            {people>1&&<p className="text-xs text-blue-400 mt-0.5">{fmt(tipPerPerson)} / person</p>}
          </div>
          <div className="bg-green-50 rounded-xl p-4 text-center">
            <p className="text-xs text-green-500 font-medium">Total {people>1?'/ Person':''}</p>
            <p className="text-2xl font-bold text-green-700 mt-1">{fmt(people>1?perPerson:total)}</p>
            {people>1&&<p className="text-xs text-green-400 mt-0.5">Total: {fmt(total)}</p>}
          </div>
        </div>
      </div>
    </ToolLayout>
