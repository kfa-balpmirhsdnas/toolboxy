          ...
          <div>
            <div className="flex justify-between mb-1">
              <div className="flex rounded overflow-hidden border border-gray-300">
                ...
                <button onClick={()=>setView('preview')} className={'px-3 py-1 text-xs font-medium transition '+(view==='preview'?'bg-blue-600 text-white':'bg-white text-gray-700 hover:bg-gray-50')}>Preview</button>
              </div>
              {view==='html'&&<button onClick={copy} className="text-xs text-blue-600 hover:underline">{copied?'Copied!':'Copy'}</button>}
            </div>
            {view==='html'?(
