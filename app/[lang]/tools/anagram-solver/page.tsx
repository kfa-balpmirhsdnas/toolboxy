    ...
    <ToolLayout tool={tool}>
      <div className="max-w-md mx-auto px-4 space-y-5">
        <div className="flex rounded-lg overflow-hidden border border-gray-300">
          <button onClick={()=>setMode('check')} className={`flex-1 py-2 text-sm font-medium transition ${mode==='check'?'bg-blue-600 text-white':'bg-white text-gray-700 hover:bg-gray-50'}`}>Check Anagram</button>
          <button onClick={()=>setMode('find')} className={`flex-1 py-2 text-sm font-medium transition ${mode==='find'?'bg-blue-600 text-white':'bg-white text-gray-700 hover:bg-gray-50'}`}>Find Anagrams</button>
        </div>
        <div><label className="block text-sm font-medium text-gray-700 mb-1">{mode==='check'?'First word':'Word to find anagrams of'}</label>
          <input value={word} onChange={e=>setWord(e.target.value)} placeholder="Enter a word..." className="w-full rounded border border-gray-300 px-3 py-2 text-lg"/></div>
        <div className="bg-gray-50 rounded-lg p-3 font-mono text-sm text-center">
          <span className="text-gray-500">Sorted letters: </span>
          <span className="text-blue-700 font-bold tracking-widest">{sorted.toUpperCase()}</span>
        </div>
        {mode==='check'?(
          <>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Second word</label>
              <input value={custom} onChange={e=>setCustom(e.target.value)} placeholder="Enter another word..." className="w-full rounded border border-gray-300 px-3 py-2 text-lg"/></div>
            {checkResult!==null&&(
              <div className={`rounded-2xl p-6 text-center ${checkResult?'bg-green-50 border-2 border-green-400':'bg-red-50 border-2 border-red-400'}`}>
                ...
                <p className={`text-xl font-bold ${checkResult?'text-green-700':'text-red-700'}`}>
                  {checkResult?'They are anagrams!':'Not anagrams'}
                </p>
              </div>
            )}
          </>
          ...
          <div>
            {anagrams.length>0?(
              <div>
                <p className="text-sm text-gray-600 mb-2">Found {anagrams.length} anagram{anagrams.length!==1?'s':''} in our word list:</p>
              ...
              <p className="text-center text-gray-400 py-4">No anagrams found in our word list for "{word}"</p>
            )}
          </div>
        )}
      </div>
