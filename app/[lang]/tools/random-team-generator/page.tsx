...
export default function RandomTeamGeneratorPage() {
  ...
  function copyAll(){
    const text=teams.map((tm,i)=>t('rtg_teamn',{n:i+1})+':\n'+tm.join('\n')).join('\n\n')
  ...
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('rtg_title')}</h1>
        <p className="text-gray-500 mb-8">{t('rtg_subtitle')}</p>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('rtg_names')}</label>
            <textarea value={names} onChange={e=>setNames(e.target.value)} rows={8}
              placeholder="Alice\nBob\nCharlie..."
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none" />
            <p className="text-xs text-gray-400 mt-1">{t('rtg_count',{n:nameCount})}</p>
          </div>
          <div className="flex items-center gap-4">
            <label className="text-sm font-medium text-gray-700 whitespace-nowrap">{t('rtg_numteams')}</label>
        ...
        {teams.length>0&&(
          <div className="mt-6">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-gray-800">{t('rtg_teams')}</h2>
              <div className="flex gap-2">
                <button onClick={generate} className="text-sm px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-lg">{t('rtg_reshuffle')}</button>
                <button onClick={copyAll} className="text-sm px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-lg">{copied?'\u2713 '+t('ui_copied'):t('ui_copy_all')}</button>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {teams.map((team,i)=>(
                <div key={i} className={'rounded-xl border p-4 '+TEAM_COLORS[i%TEAM_COLORS.length]}>
                  <h3 className="font-semibold text-gray-700 mb-2">{t('rtg_teamn',{n:i+1})} <span className="text-xs font-normal text-gray-400">{t('rtg_members',{m:team.length})}</span></h3>
