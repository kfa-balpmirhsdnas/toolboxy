...
export default function Page(){
  ...
  const tool=TOOLS.find(t=>t.slug==='text-to-handwriting')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">Ink<input type="color" value={ink} onChange={e=>setInk(e.target.value)} className="w-8 h-8 rounded cursor-pointer"/></label>
          <label className="flex items-center gap-2 text-sm text-gray-700">Paper<input type="color" value={paper} onChange={e=>setPaper(e.target.value)} className="w-8 h-8 rounded cursor-pointer"/></label>
        </div>
        <canvas ref={ref} className="w-full rounded-xl border border-gray-200"/>
        <button onClick={dl} className="px-4 py-2 bg-blue-600 text-white rounded text-sm font-medium hover:bg-blue-700">Download PNG</button>
      </div>
    </ToolLayout>
  )
}