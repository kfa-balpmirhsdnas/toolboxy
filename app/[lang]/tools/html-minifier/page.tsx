...
export default function Page(){
  ...
  const tool=TOOLS.find(x=>x.slug==='html-minifier')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('hts_input')}</label>
        ...
        <div className="flex justify-between text-xs text-gray-500 bg-gray-50 rounded px-3 py-2">
          <span>{input.length} → {output.length} {t('ui_chars')}</span><span className="text-green-600 font-semibold">{pct}% {t('hm_saved')}</span></div>
        <div><label className="block text-sm font-medium text-gray-700 mb-1">{t('cm_minified')}</label>
