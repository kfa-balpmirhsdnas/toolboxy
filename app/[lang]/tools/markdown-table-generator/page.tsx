...
export default function Page(){
  ...
  const tool=TOOLS.find(x=>x.slug==='markdown-table-generator')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        <div className="flex gap-4">
          <label className="flex items-center gap-2 text-sm text-gray-700">{t('mtt_rows')}
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">{t('mtt_cols')}
            ...
            <tbody>{cells.map((row,r)=>(
              <tr key={r} className={r===0?'bg-gray-50':''}>
                {row.map((c,ci)=><td key={ci} className="border border-gray-200 p-0">
                  <input value={c} onChange={e=>setCell(r,ci,e.target.value)} className={'w-full text-sm px-2 py-1.5 bg-transparent outline-none'+(r===0?' font-semibold':'')} placeholder={r===0?t('mtt_header'):t('mtt_cell')}/>
                </td>)}
              </tr>
            ))}</tbody>
          </table></div>
        ...
        <button onClick={()=>navigator.clipboard?.writeText(md)} className="px-4 py-2 bg-blue-600 text-white rounded text-sm font-medium hover:bg-blue-700">{t('ui_copy')}</button>
      </div>
    </ToolLayout>
  )
}