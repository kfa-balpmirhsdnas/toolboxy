...
const PRESETS=[
  {name:'Fade In',kf:'@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }',prop:'fadeIn'},
  {name:'Slide Up',kf:'@keyframes slideUp { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }',prop:'slideUp'},
  {name:'Bounce',kf:'@keyframes bounce { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-20px); } }',prop:'bounce'},
  {name:'Spin',kf:'@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }',prop:'spin'},
  {name:'Pulse',kf:'@keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.4; } }',prop:'pulse'},
  {name:'Shake',kf:'@keyframes shake { 0%,100%{transform:translateX(0)} 20%,60%{transform:translateX(-6px)} 40%,80%{transform:translateX(6px)} }',prop:'shake'},
]
export default function Page(){
  ...
  const tool=TOOLS.find(t=>t.slug==='css-animation-generator')
  return (
    <ToolLayout tool={tool}>
      <div className="max-w-lg mx-auto px-4 space-y-4">
        <div className="flex flex-wrap gap-2">
          {PRESETS.map((pr,i)=><button key={i} onClick={()=>setSel(i)} className={'px-3 py-1.5 rounded text-sm font-medium border '+(sel===i?'bg-blue-600 text-white border-blue-600':'border-gray-300 hover:bg-gray-50')}>{pr.name}</button>)}
        </div>
        <div className="grid grid-cols-3 gap-3">
          <label className="flex flex-col gap-1 text-sm text-gray-700">Duration
          ...
          <label className="flex flex-col gap-1 text-sm text-gray-700">Timing
          ...
          <label className="flex flex-col gap-1 text-sm text-gray-700">Iteration
