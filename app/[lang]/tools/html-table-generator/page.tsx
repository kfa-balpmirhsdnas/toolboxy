...
export default function Page(){
  const tool=TOOLS.find(t=>t.slug==='html-table-generator')
    ...
    <ToolLayout tool={tool}>
      <div className='space-y-4'>
        <div className='flex gap-4 flex-wrap'>
          ...
          <label className='flex items-center gap-2 text-sm'>
            Columns: <input type='number' min={1} max={10} value={cols} onChange={e=>setCols(Number(e.target.value))} className='border rounded px-2 py-1 w-16'/>
          </label>
          <label className='flex items-center gap-2 text-sm'>
