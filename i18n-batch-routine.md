# i18n 배치 이어가기 루틴 (10초 주기)

이 파일은 **다국어화(i18n) 마라톤**을 끊김 없이 이어가기 위한 루틴입니다.
10초마다 아래 **"붙여넣기 프롬프트"** 한 줄을 그대로 보내면, 클로드가 메모리를 읽고 다음 배치를 자동으로 처리·배포합니다.

---

## ▶ 붙여넣기 프롬프트 (10초마다 이 한 줄만 보내기)
...
- 10초마다 직접 보내기가 번거로우면 Claude Code 대화형 터미널에서 `/loop 배치 계속` 으로 자동 반복을 걸 수 있음 (수동 핑이 전혀 필요 없어짐).
